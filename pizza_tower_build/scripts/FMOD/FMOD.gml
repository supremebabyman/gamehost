...
This file gives gml contents to every FMOD function the Pizza Tower Switch port (v1.1.063 SR 6) uses
...
and audiogroup_special so John pillar's Meatphobia track (mu_dungeondepth1) can be turned up while the music is turned down. 
...
function fmod_sound(soundlist) 
	...
	*/
	// changed because it seems like it fucks up SFX so that it will only play if the player
	// is on the left most side of the screen
	function set_3d(emitter) {
		self.is_3d = false;
		self.emitter = fmod_emitter();
		return self;
	}
...
function get_soundlist(soundname) { 
	switch (soundname) {
		...
		case "music/pizzatime":
			return [mu_pizzatime, mu_chase, DISTASTEFUL_ANCHOVI_JC_RE_EDIT_v5c, World_Wide_Noise_v6_yo_how_many_times_am_i_just_gonna_keep_chan]
		case "music/hub":
		...
		case "music/w3/foresttitle":
			return [sound_gnome, lario_noise]
		...
		case "music/pause":
			return [Pizza_Tower___Leaning_Dream]
			// removed lario_s_secret because this is a leftover from D's Switch port
		...
		case "music/credits":
			...
			// why add this shit D?
			return [Pizza_Tower_OST___Receiding_Hairline_Celebration_Party, New_Noise_Resolutionz_v4]
		...
		// why is the noise version missing?
			return [secret_level_intro, secret_noise]
		...
		// why is there missing cases? ---------------------------
		case "sfx/playerN/punch":
			return [sfx_punch]
		case "music/pizzatimenoise":
			return [DISTASTEFUL_ANCHOVI_JC_RE_EDIT_v5c, World_Wide_Noise_v6_yo_how_many_times_am_i_just_gonna_keep_chan]
		case "music/ending":
			return [windloop]
		case "sfx/misc/pillarmusic":
			return [mu_dungeondepth2]
		case "misc/breakblock":
			return [sfx_breakblock1, sfx_breakblock2]
		case "sfx/pep/bump":	
			return [sfx_pep_mach2bump]
		case "sfx/playerN/rushdownend":
			return [sfx_playerN_rushdownhit]
		...
		case "music/soundtest/secretworld":
			return [Secret_Lockin_v1a]
		// Funni Music for Level Select
		case "music/levelselect/funni_lario":
			return [lario_s_secret]
		case "music/levelselect/titlecard":
			return [pink_panther, pink_panther]
		// basic template, as there are some sounds that are missing for some reason
		//case "":
		//	return []
...
function fmod_event_create_instance(soundpath) { 
	...
	// because its never called anywhere else and just crashes
	//global.createnumber++
	...
	switch (soundname) {
		...
		case "sfx/rat/ratsniff":
			return new fmod_sound(soundlist).set_gain(0)// you deserve this
		case "sfx/misc/thundercloud":
		case "sfx/enemies/tribaldance":
			return new fmod_sound(soundlist).set_gain(0.2)// you get to live...
			// back to good sounds
		...
		case "music/w5/kidspartytitle":
		case "music/w5/wartitle":
		// added cuz D's script never added it for some reason
		case "music/secretworldtitle":
			return new fmod_sound(soundlist).set_play_func(function () {
				default_play_func(global.fmod_is_noise_file)	
			})
		...
		case "music/title":
			return new fmod_sound(soundlist).set_create_func(function() {
				...
				if (timer != -1 && !audio_is_playing(songs[0]) 
					...
					return;
					// so for this port new_state should stay equal to 1
				if (new_state == 1) {
				...
				//if (t_state == 1) {
				// t_state doesn't do shit, so this is my shitfix
				if (global.mmenuenable == 1) {
					if timer == 0
		...
		case "music/pizzatime":
		// audio_pause_sound_insist was the problem here
			return new fmod_sound(soundlist).set_create_func(function() {
				song = noone
				noise_song = noone
				song_gain = 1;
				...
				noise_song = audio_play_sound(soundlist[2], 10, true, global.fmod_is_noise * noise_song_gain)
				...
				if (new_state == 1) {
					...
					noise_song = audio_play_sound(soundlist[2], 10, false, 0, 2 * 60 + 16.23)
					array_push(instances, song)
					array_push(instances, noise_song)
					...
					noise_song = audio_play_sound(soundlist[3], 10, true, 0)
				...
				// im too scared to remove any of this code :(
				else if (new_state == 3) {
		...
		case "music/w2/farm":
			return swap_two_tracks_intro(soundlist, [0, 0], [0, 0], 1)
		...
		//so adding a 5th arguement don't work, thanks gnome forest
			return swap_three_tracks_intro(soundlist, [0, 2 * 60 + 18.24], [0, 0], [0, 0])
		case "music/w4/industrial":
			return swap_two_tracks_intro(soundlist, [0, 2 * 60 + 23.98], [0.2, 2 * 60 + 8.19], 1)
		case "music/w4/street":
			return swap_three_tracks_intro(soundlist, [17.34, 2 * 60 + 38.04], [0.02, -1], [0, 0])
		case "music/w5/chateau":
		...
		case "music/credits":
			return new fmod_sound(soundlist).set_play_func(function () {
				// WHY IS THIS HACK JOB THE ONLY THING THAT WORKS!??!?!?
				credits = audio_play_sound(soundlist[0], 10, false, !global.fmod_is_noise_file)
				noisecredits = audio_play_sound(soundlist[1], 10, false, global.fmod_is_noise_file)
				array_push(instances, credits)
				array_push(instances, noisecredits)
		...
		// idk, it never plays the other file, so might as well
		case "music/halloweenpause":
			return new fmod_sound(soundlist).should_loop()
		/*
		...
		case "music/pause":
			return new fmod_sound(soundlist).should_loop()
			// the extra "true" arguement is the big problem in swap_two/three_tracks_intro
...
function audio_pause_sound_insist(inst) {
	// THIS IS THE CAUSE OF MY MENTAL DECLINE!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	// OF COURSE THIS OF ALL THINGS IS CAUSING CRASHES!
	// WE MUST NOT INSIST, D!
	// WE MUST NOT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
