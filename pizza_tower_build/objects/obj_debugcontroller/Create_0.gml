...
if DEBUG
{
	active = false;
	showoverlay = false;
	SET_GAME_SPEED = new DebugCommand("SETGAMESPEED", "Sets the game speed", "<int>", function(int)
	{
		game_set_speed(floor(real(int)), gamespeed_fps);
	});
	TEST_P_RANK = new DebugCommand("TESTPRANK", "", "", function()
	{
	...
	KILL_BOSS = new DebugCommand("KILLBOSS", "", "", function()
	{
		instance_destroy(obj_baddie);
	});
	SET_BOSS_HP = new DebugCommand("SETBOSSHP", "", "<int>", function(int)
	{
		with obj_baddie
			elitehit = real(int);
	});
	SWITCH_CHAR = new DebugCommand("SWITCHCHAR", "Switches character", "", function()
	{
		with obj_player
		{
			character = "P";
	...
	SHOW_DEBUG_OVERLAY = new DebugCommand("SHOWDEBUGOVERLAY", "Toggles debug overlay", "", function()
	{
	...
	UNLOCK_TOPPINS = new DebugCommand("UNLOCKTOPPINS", "Unlocks the toppins of the given level", "<level_name_ini>", function(level_name_ini)
	{
	...
	UNLOCK_BOSS_KEY = new DebugCommand("UNLOCKBOSSKEY", "Unlocks the boss key of the given world (1, 2, 3, 4)", "<number>", function(number)
	{
	...
	NOCLIP = new DebugCommand("NOCLIP", "NoClip through the Tower", "", function()
	{
		with obj_player1
	...
	SETCOMBO = new DebugCommand("SETCOMBO", "Set the combo", "<combo> <combotime:optional>", function(combo, combotime)
	{
		if combo == undefined
			exit;
	...
	ALLTOPPINS = new DebugCommand("ALLTOPPINS", "Gives all the toppins", "", function()
	{
	...
	PLAYER_ROOM = new DebugCommand("PLAYERROOM", "Go to a given room", "<targetRoom> <targetDoor>", function(_room, _door)
	{
		if _room == undefined
			exit;
		if _door == undefined
	...
	CAMERA_ZOOM = new DebugCommand("CAMERAZOOM", "Zoom", "<targetzoom> <spd>", function(zoom, spd)
	{
		if zoom == undefined
			exit;
		if spd != undefined
	...
	HARDMODE = new DebugCommand("HARDMODE", "Toggles Hard Mode", "<bool>", function(toggle)
	{
		if toggle == undefined
			exit;
		toggle = get_bool(toggle);
	...
	PLAYER_SET_STATE = new DebugCommand("PLAYERSETSTATE", "Changes the player state", "<states.state>", function(_state)
	{
		if _state == undefined
			exit;
	...
	// LEVEL_SELECT is a custom command meant to send player to the Level Select Screen
	LEVEL_SELECT = new DebugCommand("LEVELSELECT", "EXPERIMENTAL - Go to Level Select", "", function(_select)
	{
		if room == Mainmenu
			exit;
		else {
			DoCommand("PLAYERROOM NEWLEVELSELECT A");
		}
	});
	...
	ds_list_add(command_list, DESTROYICE, SET_GAME_SPEED, SHOW_HUD, SHOW_COLLISIONS, PLAYER_ROOM, CAMERA_ZOOM, HARDMODE, PLAYER_SET_STATE, PANIC, ALLTOPPINS, GIVEHEAT, ROOMCHECK, SWITCH_CHAR, SET_BOSS_HP, LEVEL_SELECT);
