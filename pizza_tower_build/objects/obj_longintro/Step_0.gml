...
switch scene
{
	case -2:
		if (_switch && !instance_exists(obj_logoprop))
		{
			...
			{
				sprite_index = burnedshit_intro;
				depth = -1;
				audio_play_sound(hazardous_environments, 10, false, 2);
				//image_xscale = 0.5;
	...
	case -1:
		if (_switch && !instance_exists(obj_logoprop))
		{
			if audio_exists(hazardous_environments)
				audio_sound_gain(hazardous_environments, 0, 3000);
