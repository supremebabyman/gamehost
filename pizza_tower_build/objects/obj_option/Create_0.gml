...
// custom credits
if global.creditslocked == 0 {
add_option_press(categories, 4, "option_credits", function()
{
	instance_create(0, 0, obj_customcredits);
});
...
// for enabling unlocking all palettes and patterns
// Can only be Toggled in Main Menu because it only checks once you load a Save
if (global.creditslocked == 0) {
	add_option_toggle(game_menu, 6, "option_pal_toggle", function(val)
	{
		ini_open_from_string(obj_savesystem.ini_str_options);
		ini_write_real("Option", "pal_toggle", val);
		obj_savesystem.ini_str_options = ini_close();
		global.clothes_allunlock = val;
	
		// in clothes_options
		all_clothes_unlock();

	}).value = global.clothes_allunlock;
}
