...
ds_map_set(pause_menu_map, "pause_restart", [2, function()
{
	...
	if !global.snickchallenge
	{
		...
		if rm != -4 && rm != -1
		{
			...
			instance_destroy(obj_keyconfig);
			pause = false;
			// gonna keep stop_music since it should fix music getting louder every restart
			// still kinda does it, but stops after another restart and never gets out of hand
			restart_thy_music();
		}
		else
			fmod_event_one_shot("event:/sfx/ui/select");
